import { layoutNextLineRange, materializeLineRange, measureNaturalWidth, prepareWithSegments, } from './layout.js';
...
function stepInlineFlowLine(flow, maxWidth, start, collectFragment) {
    ...
    lineLoop: while (itemIndex < flow.items.length) {
        ...
        if (item.break === 'never') {
            ...
            if (lineWidth > 0 && totalWidth > remainingWidth)
                break lineLoop;
            collectFragment?.(item, gapBefore, occupiedWidth, cloneCursor(EMPTY_LAYOUT_CURSOR), cloneCursor(item.end));
        ...
        if (atItemStart) {
            ...
            if (totalWidth <= remainingWidth) {
                collectFragment?.(item, gapBefore, item.naturalWidth + item.extraWidth, cloneCursor(EMPTY_LAYOUT_CURSOR), cloneCursor(item.end));
                lineWidth += totalWidth;
                remainingWidth = Math.max(0, safeWidth - lineWidth);
                itemIndex++;
                textCursor = EMPTY_LAYOUT_CURSOR;
        ...
        collectFragment?.(item, gapBefore, line.width + item.extraWidth, cloneCursor(textCursor), cloneCursor(line.end));
...
export function layoutNextInlineFlowLineRange(prepared, maxWidth, start = FLOW_START_CURSOR) {
...
export function layoutNextInlineFlowLine(prepared, maxWidth, start = FLOW_START_CURSOR) {
    ...
    return {
        fragments: line.fragments.map(fragment => {
            ...
            if (item === undefined)
                throw new Error('Missing inline-flow item for fragment');
            return {
                ...fragment,
...
export function walkInlineFlowLineRanges(prepared, maxWidth, onLine) {
    let lineCount = 0;
...
export function measureInlineFlowGeometry(prepared, maxWidth) {
    const flow = getInternalPreparedInlineFlow(prepared);
    let lineCount = 0;
    let maxLineWidth = 0;
    let cursor = FLOW_START_CURSOR;
