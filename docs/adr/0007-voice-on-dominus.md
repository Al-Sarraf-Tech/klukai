...
at `192.168.50.2:8301`. `companion-core` on amarillo proxies TTS
requests via LAN. Per `feedback_dominus_voice_port.md`: the voice
...
- **No iPhone/local TTS**: rejected — Klukai's voice must be consistent
  across all clients.
- **LAN-only path** per `feedback_lan_transfers.md`: voice latency is
  user-perceptible (>50ms feels laggy); Tailscale's ~5-10ms LAN penalty
  is too much.
